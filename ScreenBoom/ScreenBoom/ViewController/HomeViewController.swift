...
class HomeViewController: BaseViewController, UIViewControllerTransitioningDelegate {
    ...
    func loadData() {
        FireBaseManager.sharedInstance.getEventsCount { (eventCountResult) in
            switch eventCountResult {
            case.Failure(let error):
                print(error)
                break
            case.Success(let eventCount):
                FireBaseManager.sharedInstance.getUserCount(completion: { (userCountResult) in
